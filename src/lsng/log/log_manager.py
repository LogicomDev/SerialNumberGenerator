...
class LogManager(object):
    ...
    def add_trace(self, to_write, scope=logging.DEBUG):
        
        timing = str(datetime.datetime.now())
        
        self.tracelog.append((to_write, scope, timing))
        
        rows = self.parent.uic.txt_log.rowCount()
        self.parent.uic.txt_log.setRowCount(rows + 1)

        if scope == logging.DEBUG:
            color = Qt.black
            pixmap = QPixmap("")
        elif scope == logging.INFO:
            color = Qt.green
            iconpath = os.path.join(RESSOURCE_PATH, "log_info.png")
            pixmap = QPixmap(iconpath)
        elif scope == logging.CRITICAL:
            color = Qt.red
            iconpath = os.path.join(RESSOURCE_PATH, "log_critical.png")
            pixmap = QPixmap(iconpath)
        elif scope == logging.WARNING:
            color = Qt.blue
            iconpath = os.path.join(RESSOURCE_PATH, "log_warning.png")
            pixmap = QPixmap(iconpath)
        elif scope == logging.ERROR:
            color = QColor("Orange")
    ...
    def add_trace_with_box(self, msg, scope=logging.INFO):
        if scope == logging.INFO:
